...
The original committed paper outputs were restored, and the page was rebuilt from those outputs. The targeted distribution and paper-output tests then passed: 29 passed, with the byte-for-byte regeneration test excluded. The browser-file assertion now includes the distribution engine and interface. The full suite has not been rerun to a clean pass; the original byte-for-byte reproducibility gate remains unchanged.
